...
class TwoFusion_net(nn.Module):
    ...
    def forward(self, input):
        input, mod_pad_w_ivif, mod_pad_h_ivif = self.check_image_size(input);    
        #print(mod_pad_w_ivif)
        #print(mod_pad_h_ivif)
        
        out_features = self.encoder(input)
        out_f = self.decoder(out_features)
        
        _, _, h, w = out_f.size()
        out_f = out_f[:, :, :h - mod_pad_h_ivif, :w - mod_pad_w_ivif]
        return out_f